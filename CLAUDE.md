...
- **抓取双通道**：Textractor（内存钩子，主）+ OCR（截屏，兜底）。OCR 后端二选一：本地 `Windows.Media.Ocr`（winrt，离线免费）或 AI 视觉大模型。不自研钩子引擎；**已弃用 PaddleOCR**（3.x 模型下载链路坏、依赖重，换 winrt）。
- **语言**：中文，无翻译层（用户主要玩中文版）。
...
- OCR：`winrt-*` 系列（Windows.Media.Ocr，Python 3.13 需 v3.x 子包）+ `mss` 截屏 + `ctypes` PrintWindow 抓窗口（被遮挡也能抓）
...
v1 MVP 已跑通核心管线：文件回放注入器（模拟抓取）→ 去重 → transcript 落盘 → DeepSeek 自动整理结构化笔记 → 剧情对话 + 洞察回填 → 路标。v2 接 Textractor 真实抓取。v3 接 OCR（winrt + AI 视觉）、向量检索、多模型独立配置。34 个单元测试全绿。
...
- ~~OCR 通道~~ ✅ v3（winrt + AI 视觉，已弃用 PaddleOCR）
